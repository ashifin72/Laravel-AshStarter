...
@section('content')
    ...
    <section class="content">
        <!-- Small boxes (Stat box) -->
            ...
            <form class="row  main_localization" action="{{route('main_currency')}}" method="post">
                @csrf
                <div class="form-group col-6">

                    <label for="title">Валюта по умалчиванию</label>
        ...
        <div class="card-body table-responsive p-0 card">
            <table class="table table-hover text-nowrap">
                ...
                <tr>
                    ...
                    <th class="text-center">{{__('Курс')}}</th>
                    <th>{{__('admin.status')}}</th>
                    <th class="text-center">{{__('admin.favorite')}}</th>
                    <th>
                        <a class="btn btn-outline-primary" href="{{route('admin.currencies.create')}}">{{__('добавить')}}</a>
                    </th>
                </tr>
                </thead>
                <tbody>
                @forelse($items as $item)
                    <tr @if($item->status == 0) style="background: #e0e0e0"   @endif>
                        ...
                        <td class="text-center">
                            <span class="tag tag-success">{{$item->value}}</span>
                        </td>
                        <td class="text-center">
                            @if ($item->status == 1)
                        ...
                        <td>
                            <a class="btn btn-outline-primary" href="{{route('admin.currencies.edit', $item->id )}}">{{__('Изменить')}}</a>
                        </td>
                    </tr>
                @empty
                    <tr>
